package com.example.base_project.wave_form.soundeditor.widget;
...
import com.example.base_project.wave_form.soundeditor.utils.SelectionPoint;
...
public class WaveformView extends View {
    ...
    public WaveformView(Context context, AttributeSet attrs) {
        ...
        mGridPaint.setColor(getResources().getColor(R.color.editor_grind_paint_color, null));

        mSelectedLinePaint = new Paint();
        mSelectedLinePaint.setAntiAlias(false);
        mSelectedLinePaint.setStrokeWidth(pxtodp(2));
        mSelectedLinePaint.setStrokeCap(Paint.Cap.ROUND);
//        mSelectedLinePaint.setColor(getResources().getColor(R.color.editor_selected_line_paint_color));
        mSelectedLinePaint.setShader(new LinearGradient(0, 0, 0, getHeight(), Color.parseColor("#4CAF50"), Color.parseColor("#4CAF50"), Shader.TileMode.MIRROR));
        mUnselectedLinePaint = new Paint();
        mUnselectedLinePaint.setAntiAlias(false);
        mUnselectedLinePaint.setStrokeWidth(pxtodp(1));
        mUnselectedLinePaint.setStrokeCap(Paint.Cap.ROUND);
        mUnselectedLinePaint.setColor(getResources().getColor(R.color.editor_mUnselectedline_color, null));
        /*mUnselectedLinePaint.setShader(new LinearGradient(0, 0, 0, getHeight(),
                                                        Color.parseColor("#3F4653"),
                                                        Color.parseColor("#3F4653"), Shader.TileMode.MIRROR));*/
    ...
    public void setSoundFile(CheapSoundFile soundFile) {
        ...
        if (mSoundDuration < 5000) {
            postDelayed(() -> {
                ...
                if (endpoint - startpoint < 40000) {
                    double temp = endpoint + (40000 - (endpoint - startpoint));
                    ...
                    if (divider != 0 && h != 0) {
                        try {
                            if (((h / (divider / 100)) > 0) && (h < getMeasuredHeight() / 2)) {
                                int startSec = pixelsToMillisecs(i);
                                SelectionPoint data = new SelectionPoint(i, h, startSec, 0);
                                if (startSec >= 0) dataPointsArrayList.add(data);
                            }
                        } catch (java.lang.ArithmeticException ex) {
                            Log.e("Exception", "ArithmeticException");
                        }
                    }
                }
            }
    ...
    public void zoomIn() {
        if (canZoomIn()) {
            ...
            mSelectionStart *= (int) factor;
            mSelectionEnd *= (int) factor;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isSplit) {
            canvas.drawLine(mSelectionStart - mOffset + 0.5f, 0, mSelectionStart - mOffset + 0.5f, measuredHeight, mBorderLinePaint);
        }

        canvas.drawLine(mSelectionEnd - mOffset + 0.5f, 0, mSelectionEnd - mOffset + 0.5f, measuredHeight, mBorderLinePaint);

        canvas.drawRect(mSelectionStart - mOffset + 0.5f, 0, mSelectionEnd - mOffset + 0.5f, measuredHeight, mSelectedRectangle);
...
//        canvas.drawRect(0, 0, getMeasuredWidth(), pxtodp(1), mGridPaint);
        ...
        while (i < width) {
            ...
            if (integerTimecodeNew != integerTimecode) {
                ...
                int minutes = integerSecs2 / 60;
                int seconds = integerSecs2 % 60;
                ...
                if (i < (getMeasuredWidth() - getMeasuredWidth() / 14) && getZoomLevel() == 0) {
                    canvas.drawText(timecodeStr, i - offset, (int) (16 * mDensity + 20), mTimecodePaint);
                ...
                /*if (seconds % 5 != 0) {
                    canvas.drawText(".", i, (int) (16 * mDensity + 120), mTimecodePaint);
                }*/
            }
        }
    ...
    private void drawWaveformPlayed(final Canvas canvas, final int i, final int start, final int measuredHeight) {
        if (i + start == mPlaybackPos && mPlaybackPos != -1) {
            canvas.drawLine(i, minHeight40, i, measuredHeight, mPlaybackLinePaint);
        }
    }
    ...
    private void drawWaveformUnPlayed(final Canvas canvas, final int i, final int start, final int ctr, final Paint paint) {
        ...
        if ((((start + i)) % mSamplingRate) == 0) {
            ...
            drawWaveformLine(canvas, i, positivey, negativey, paint);
        }
