...
public class SoundFile// A SoundFile object should only be created using the static methods create() and record().
public constructor() {
...
//        }
    ...
    private fun ReadFile(inputFile: File) {
        ...
        while (i < numFrames) {
            ...
            frameGains!![i] = sqrt(gain.toDouble()).toInt()  // here gain = sqrt(max value of 1st channel)...
    ...
    @Throws(IOException::class)  fun WriteFile(outputFile: File, startTime: Float, endTime: Float) {
    ...
    companion object {
        ...
        fun create(fileName: String,
            ...
            val name = f.name.lowercase(getDefault())
            val components = name.split("\\.".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            if (components.size < 2) {
                return null
            }
            if (!listOf(*supportedExtensions).contains(components[components.size - 1])) {
                return null
            }
