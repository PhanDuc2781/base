...
//            if (i <= fadeTime && fadeIn) {
//                out.write(fade(buffer, 1 / fadeTime * i), 0, len);
//            } else if (i > numFrames - fadeTime && fadeOut) {
//                out.write(fade(buffer, 1 / fadeTime * i), 0, len);
//            } else {
...
//            }
    ...
    private byte[] fade(byte[] buffer, double scale) {
        ...
        if ((buffer[3] & 0xC0) == 0xC0) {
            ...
            mGlobalChannels = 2;
            if (mpgVersion == 1) {
                int value = (buffer[9] & 0x7F) << 1;
                int sign = (buffer[10] & 0x80) >> 7;

                value = (int) (value * scale);
                value = value & 0x7F;

                buffer[9] = (byte) (((byte) (value >> 1)) | (sign << 7));
            } else {
                gain = 0;  // ???
            }
        }
    ...
    static private final int[] BITRATES_MPEG1_L3 = {
            0, 32, 40, 48, 56, 64, 80, 96,
            112, 128, 160, 192, 224, 256, 320, 0};
    static private final int[] BITRATES_MPEG2_L3 = {
            0, 8, 16, 24, 32, 40, 48, 56,
            64, 80, 96, 112, 128, 144, 160, 0};
    static private final int[] SAMPLERATES_MPEG1_L3 = {
            44100, 48000, 32000, 0};
    static private final int[] SAMPLERATES_MPEG2_L3 = {
            22050, 24000, 16000, 0};
}
