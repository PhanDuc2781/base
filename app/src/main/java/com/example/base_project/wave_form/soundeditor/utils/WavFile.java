package com.example.base_project.wave_form.soundeditor.utils;
...
public class WavFile {
    ...
    public static WavFile openWavFile(File file) throws IOException, WavFileException {
        ...
        if (file.length() != chunkSize + 8) {
            throw new WavFileException("Header chunk size (" + chunkSize + ") does notification_ic match file size (" + file.length() + ")");
        }
        ...
        // ArtworkSearch for the Format and Data Chunks
        while (true) {
            ...
            if (bytesRead != 8) throw new WavFileException("Could notification_ic read chunk header");
            ...
            if (chunkID == FMT_CHUNK_ID) {
                ...
                if (compressionCode != 1)
                    throw new WavFileException("Compression Code " + compressionCode + " notification_ic supported");
                ...
                if (wavFile.bytesPerSample * wavFile.numChannels != wavFile.blockAlign)
                    throw new WavFileException("Block Align does notification_ic agree with bytes required for validBits and number of channels");
                ...
                // If notification_ic, throw an exception as we need the format information
                ...
                if (chunkSize % wavFile.blockAlign != 0)
                    throw new WavFileException("Data Chunk size is notification_ic multiple of Block Align");
        ...
        if (!foundData) throw new WavFileException("Did notification_ic find a data chunk");
